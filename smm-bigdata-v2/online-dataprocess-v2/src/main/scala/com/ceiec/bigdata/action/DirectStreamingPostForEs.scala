...
import com.ceiec.bigdata.entity.{AllFacebookPost, AllNewsPost, UserEntityCommon}
...
object DirectStreamingPostForEs {
                    ...
                    userType match {
                      ...
                      case "1" => {
                        ...
                        if (twitterRoot.getLang != null) {
                          ...
                          //拿到twitter粉丝数
                          var twitterFansNub :Int = twitterRoot.getUser.getFollowers_count
                          if (twitterFansNub == null) {twitterFansNub = 0}
                      ...
                      case "2" => {
                        ...
                        var facebookPost : AllFacebookPost= null
                        try{

                           facebookPost  = JSON.parseObject(facebookPostStr, classOf[AllFacebookPost])
                        }catch {
                          case ex: Exception => {
                            log.error("fb json prase error :", ex)
                          }
                        }

                        if (facebookPost != null && facebookPost.getContent != null) {
                          //判断是否符合写入es的条件
