...
/**
  ...
  */
object DirectStreamingTest extends Serializable {
  ...
  def functionToCreateContext(args: Array[String]): StreamingContext = {
    val sparkConf = new SparkConf().setAppName("test-post-v2").setMaster("local[*]")
    ...
    val topicsSet = "InsightCyber_post_3".split(",").toSet //diplomat_twitter,diplomat_news,diplomat_twitter_streaming,diplomat_twitter_user,diplomat_facebook
    val kafkaParams = scala.collection.immutable.Map[String, String]("bootstrap.servers" -> "172.16.3.31:6667,172.16.3.32:6667,172.16.3.33:6667", "auto.offset.reset" -> "largest", "group.id" -> "test_video") //largest  smallest
...
//              println(userEntityCommon.getContent)
              linetype match {
                //处理帖子信息
                case _ => {
                  userType match {
                    ...
                    case "2" => {
                      ...
                      //判断是否符合写入es的条件
                      val pojoString: String = transNlpPojoToString(String.valueOf(facebookCount), "auto", facebookPost.getContent)
                    ...
                    case _ => {
                      ...
                      if (allNewsPost.getContent != null && (!allNewsPost.getContent.trim.equals("")) && allNewsPost.getTitle != null) {
                        ...
                        if (allNewsPost.getLang != null) {
                          pojoString = transNlpPojoToString(String.valueOf(newsCount), allNewsPost.getLang, allNewsPost.getContent)
                        }
                        //当没有语言时提交文本处理
                        else {
                          pojoString = transNlpPojoToString(String.valueOf(newsCount), "auto", allNewsPost.getContent)
                        }
