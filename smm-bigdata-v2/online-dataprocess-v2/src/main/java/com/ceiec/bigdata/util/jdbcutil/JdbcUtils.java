package com.ceiec.bigdata.util.jdbcutil;
...
public class JdbcUtils {
    ...
    static org.apache.log4j.Logger logger=org.apache.log4j.Logger.getLogger(JdbcUtils.class);


    public  Connection getConnection() {
        String driver = "com.mysql.jdbc.Driver";
        String url = "jdbc:mysql://172.16.3.35:3306/smm_v1.1";
     ...
     * @throws Exception
     */
    public <T> List<T> findSiteInfoResult(String sql, List<Object> params,
                                         Class<T> cls )throws Exception {
        try {
        List<T> list = new ArrayList<T>();
        int index = 1;
...
//                        cols_value = "";
//                    }
                    if(cols_name.trim().equals("url")){
                        cols_value = UrlFilter.filterUrl((String) resultSet.getObject(cols_name));
                    }
     ...
     * @throws Exception
     */
    public <T> List<T> findResultWithConn(Connection connection ,String sql, List<Object> params,
                                          Class<T> cls )throws Exception {
        try {
            List<T> list = new ArrayList<T>();
            int index = 1;
     ...
     * @throws Exception
     */
    public <T> List<T> findRegionInfiToLowResultWithConn(Connection connection ,String sql, List<Object> params,
                                          Class<T> cls )throws Exception {
        try {
            List<T> list = new ArrayList<T>();
            int index = 1;
            ...
            while(resultSet.next()){
                ...
                for(int i = 0; i<cols_len; i++){
                    ...
                    field.setAccessible(true); //打开javabean的访问权限
                    if(resultObject != null){
                        if(cols_value != null && cols_name.toString().equals("full_name")){
                            field.set(resultObject, cols_value.toString().toLowerCase());
                        }else {
                            field.set(resultObject, cols_value);
                        }
